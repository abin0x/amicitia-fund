...
export default function OnboardingScreen({ onFinish }: { onFinish: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] overflow-hidden bg-[radial-gradient(circle_at_top,#effaf4_0%,#edf6f2_38%,#f8fbfa_100%)]">
      ...
      <div className="relative mx-auto flex min-h-screen max-w-md flex-col px-5 pb-8 pt-10">
        <div className="flex items-center justify-between">
          <img src="/amicitia-logo.png" alt="Amicitia logo" className="theme-logo h-12 w-auto max-w-[190px] object-contain" />
          <button
            type="button"
            onClick={onFinish}
            className="rounded-full border border-border/70 bg-card/80 px-4 py-2 text-sm font-medium text-muted-foreground shadow-sm transition-colors hover:text-foreground"
          >
            Skip
          </button>
        </div>
        ...
        <div className="flex flex-1 flex-col justify-center">
          <div className="rounded-[32px] border border-border/70 bg-card/80 p-6 shadow-[0_24px_60px_rgba(16,24,40,0.12)] backdrop-blur-2xl">
            <div className="flex h-16 w-16 items-center justify-center rounded-[24px] bg-primary/8 text-primary shadow-sm">
              ...
              {slides.map((_, index) => (
                <button
                  ...
                  className={`h-2.5 rounded-full transition-all ${
                    index === activeSlide ? "w-8 bg-primary" : "w-2.5 bg-primary/20"
                  }`}
        ...
        <div className="space-y-3">
          <Button
            type="button"
            onClick={goNext}
            className="h-12 w-full rounded-2xl text-base font-semibold shadow-[0_16px_32px_rgba(20,102,76,0.20)]"
          >
